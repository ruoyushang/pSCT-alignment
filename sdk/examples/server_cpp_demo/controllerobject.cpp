  ...
  m_pMethodStop(NULL)
{
    ...
    // Add state alarm condition "StateCondition" as compontent to the object
    ...
    UA_ASSERT(addStatus.isGood());
    // Set historizing to false, logging is deactivated by default
    pAnalogItem->setHistorizing(OpcUa_False);
    ...
    // Removed: Change value handling to get read and write calls to the node manager
    //pAnalogItem->setValueHandling(UaVariable_Value_Cache);
    // Added: Set value handling to handle sampling on request
    pAnalogItem->setValueHandling(UaVariable_Value_CacheIsSource | UaVariable_Value_CacheIsUpdatedOnRequest);
    ...
    if(pMethodHandleUaNode)
    {
        ...
        if(pMethod)
        {
            ...
            if ( ret.isGood() )
            {
                ...
                eventData.m_State.setInt32(state);
                ...
                eventData.setMessage(UaLocalizedText("en", messageText));
