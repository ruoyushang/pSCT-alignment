...
read -r -p "Now we plan to extend 800 steps and retract 800 steps. Are you sure? [y/n] " response
if [[ "$response" =~ ^([yY][eE][sS]|[yY])+$ ]]
then
    echo "========"
...
read -r -p "Now do you want to do the same with high current mode?? (Will extend 800 steps and retract 800 steps in high current mode.) Are you sure? [y/n] " response
if [[ "$response" =~ ^([yY][eE][sS]|[yY])+$ ]]
then
    echo "========"
