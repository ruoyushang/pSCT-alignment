...
from bokeh.models import ColumnDataSource, ColorBar, CDSView, BooleanFilter
from bokeh.palettes import viridis, RdBu9
...
def read_misalignment(inputfilename="logs/p2pasclient_log_20190703184623.txt", out_text="mpes_readings_new.txt"):
    ...
    if plot:
        plotfile = "alignment_plot_" + outputfilename.split('.')[0].split('_')[-1] + "reading_" + str(num_read) + ".png"
        #plot_misalignments(all_misalignment, comparison, zmax=1, outfilename=plotfile)
        bokeh_plot_misalignments(all_misalignment, "reading_" + str(num_read), comparison, zmax=1, outfilename=plotfile)
        print("Saving plot to {}".format(plotfile))
        plotfile2 = "alignment_plot_" + outputfilename.split('.')[0].split('_')[-1] + "reading_" + str(
            num_read) + "_hist.png"
        plt.figure(figsize=(8, 3))
...
def bokeh_plot_misalignments(all_misalignments, title, comparison=False, zmax=None,outfilename="misalignment.png"):
    data = ColumnDataSource(pd.DataFrame(all_misalignments).transpose())
    #print(all_misalignments)
    booleans = [True if z_val < 1 else False for z_val in data.data['misalignment']]
    view = CDSView(source=data, filters=[BooleanFilter(booleans)])
    tools = ["box_select", "hover", "reset", "save"]
    tooltips = [("MPES", "@index"), ("Offset", "@misalignment"), ("Edge", "@edge"), ("Port", "@port")]
    if zmax is None:
        zmax = max(data.data['misalignment'])

    output_file("test.html", title="Misalignments")

    if comparison:
        p = figure(plot_width=500, plot_height=500, title="Changed Misalignments of " + title, tools=tools,
                   tooltips=tooltips)
        mapper = linear_cmap("misalignment", palette=RdBu9, low=-0.2, high=0.2)
        p.circle('coor_x', 'coor_y', size=12, source=data, color=mapper, hover_color='red')
        color_bar = ColorBar(color_mapper=mapper['transform'], width=8, location=(0, 0), title="Misalignment [mm]",label_standoff=12)
        p.add_layout(color_bar, 'right')

        show(p)
    else:
        #p = figure(plot_width=500, plot_height=500, title="Misalignments of " + title, tools=tools, tooltips=tooltips)
        p = figure(plot_width=500, plot_height=500, title="Misalignment [mm]", tools=tools, tooltips=tooltips, match_aspect=True, title_location="right")
        mapper = linear_cmap("misalignment", palette=viridis(6), low=0, high=zmax)
        p.circle('coor_x', 'coor_y', size=8, source=data, color=mapper, hover_color='red')
        color_bar = ColorBar(color_mapper=mapper['transform'], label_standoff=12,width=12, location=(0, 0))#, title="Offset [mm]")
        color_bar_plot = figure(title="Misalignment [mm]", title_location="right",
                        height=500, width=100,
                        toolbar_location=None, min_border=0,
                        outline_line_color=None)

        #p.add_layout(color_bar_plot, 'right')
        p.add_layout(color_bar, 'right')
        p.axis.visible = False
        p.xgrid.visible = False
        p.ygrid.visible = False

        #color_bar_plot.add_layout(p, 'right')
        color_bar_plot.title.align="center"
        color_bar_plot.title.text_font_size = '12pt'

        #p.add_layout(color_bar_plot,'right')
        p_filtered = figure(plot_width=500, plot_height=500, title="Small Misalignments (<1mm) of " + title,
                            tools=tools, tooltips=tooltips)
        mapper_small = linear_cmap("misalignment", palette=viridis(6), low=min(data.data["misalignment"]), high=1)
        p_filtered.circle('coor_x', 'coor_y', size=12, source=data, color=mapper_small, hover_color='red', view=view)
        color_bar_small = ColorBar(color_mapper=mapper_small['transform'], width=8, location=(0, 0), title="mm Offset")
        p_filtered.add_layout(color_bar_small, 'right')
        p.toolbar.logo = None
        ...
        export_svgs(p, filename=outfilename[:-3]+'svg')
        #export_png(layout, filename=outfilename)
        #export_svgs(layout, filename=outfilename)
