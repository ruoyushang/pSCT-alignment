...
class pSCTDB_readonly:
    ...
    def query_port_info(self, PEDB_port):
        if PEDB_port<0 or PEDB_port>16:
            print("PEDB_port should be between 0 and 16, are you sure what you are doing?")
            exit(0)
        self.connect()
        print("====")
        print((PEDB_port-1)*3+1, PEDB_port*3)
        queryM1 = "select position, mpcb_id, mpcb_ip_address, pedb_channel from Opt_MPMMapping where pedb_channel >= {} and pedb_channel <= {} and position like '1%' order by pedb_channel;".format( (PEDB_port-1)*3+1, PEDB_port*3 )
        queryM2 = "select position, mpcb_id, mpcb_ip_address, pedb_channel from Opt_MPMMapping where pedb_channel >= {} and pedb_channel <= {} and position like '2%' order by pedb_channel;".format( (PEDB_port-1)*3+1, PEDB_port*3 )
        nentries=self.cur.execute(queryM1)
        print("Primary panels plugged into this port: ")
        for i, row in enumerate(self.cur):
            #ips.append(row[0])
            print("Panel {} IP {} PEDB Channel {} port {}".format(row[0], row[2], row[3],PEDB_port))
        if PEDB_port <= 8:
            nentries2=self.cur.execute(queryM2)
            print("====")
            print("Secondary panels plugged into this port: ")
            for i, row in enumerate(self.cur):
                print("Panel {} IP {} PEDB Channel {} port {}".format(row[0], row[2], row[3],PEDB_port))
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Print out ALL panels plugged into the provided port')
    parser.add_argument('port', default='1', help="Port ID", type=int)
    parser.add_argument('-H', '--host', default='172.17.10.10', help="Host for DB", type=str)
    args = parser.parse_args()

    testDB = pSCTDB_readonly(args.host)
    testDB.query_port_info(args.port)
