...
//Set and initialize a few default values
    ...
    if (argc == 1) {//use default values for camera
        //change nothing
    } else if (argc >= 2) {
        for (int j = 1; j < argc; j++) {
            if (!strcmp(argv[j], "-f")) {
                ...
                verbose = true;
                leds.VERBOSE = true;
            } else if (!strcmp(argv[j - 1], "-f") ||
                       ((!strcmp(argv[j - 1], "-s") || !strcmp(argv[j - 1], "-A")) && (argv[j][0] != '-'))) {
                ...
                cout
                        << "-s (save fits or raw image with current datetime.fits/raw as file name, default = does not save)"
                        << endl;
                cout << "-v (verbose output on , default = verbose output off)" << endl;
                cout << "-r <#> (repeat <#> times sequentially, -1 repeats without end, 0 takes no images, default = 1)"
                     << endl;
...
//Output final results
        ...
        if (!captureonly) {
            ...
            cout << "@OUTPUT " << ledsout->TIME << " " << ledsout->SPACE[0] << " " << ledsout->SPACE[1] << " "
                 << ledsout->SPACE[2] << " " << ledsout->SPACE[3] << " " << ledsout->SPACE[4] << " "
                 << ledsout->SPACE[5] << endl;
        }
    }
