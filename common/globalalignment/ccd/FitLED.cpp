...
FitLED::FitLED(LEDoutputs* lo): ledsin(lo->inleds), ledsout(lo){
	...
	x[0] = 0;x[1] = 0;x[2] = 8500;
	x[3] = 0;x[4] = 0;x[5] = PI/2.0;
	double minf;
        opt.optimize(x, minf);
...
double** FitLED::moveled(vector<double> p){
	...
	for(int i = 0; i < ledsin->NLED; i++){
		for(int j =0; j<3; j++){
			...
			w[i][j]=p[j]+(rot[j][0]*ledsin->LED[i][0]+rot[j][1]*ledsin->LED[i][1]+rot[j][2]*ledsin->LED[i][2]); 
		}
	}
...
double f_ledfit(const std::vector<double> &x, std::vector<double> &grad, void* f_data){
	...
	for (int i=0; i<thetest->getLEDinputs()->NLED; i++){
			if(thetest->getLEDoutputs()->LEDSPRESENT[i]) offset+=pow(ledsf[i][0]-y[i][0],2)+pow(ledsf[i][1]-y[i][1],2);
	}
	return offset;
