...
void Image::savefilteredFITSImage(const LEDoutputs* lo, const char strTime[16]) {
    ...
    for(int i=0;i < lo->inleds->NLED; i++)
        ...
        if(lo->LEDSPRESENT[i])
            {
            pFits->pHDU().addKey("LEDPOS"+to_string(i)+"X", lo->LEDPOS[i][0], "X Pixel location of LED "+to_string(i));
            pFits->pHDU().addKey("LEDPOS"+to_string(i)+"Y", lo->LEDPOS[i][1], "Y Pixel location of LED "+to_string(i));
            }
        }
