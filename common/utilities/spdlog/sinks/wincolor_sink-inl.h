...
namespace spdlog {
    namespace sinks {
        ...
        SPDLOG_INLINE wincolor_sink<TargetStream, ConsoleMutex>::wincolor_sink(color_mode mode)
            : out_handle_(TargetStream::handle()), mutex_(ConsoleMutex::mutex()) {
            set_color_mode(mode);
...
// change the color for the given level
        ...
        template<typename TargetStream, typename ConsoleMutex>
        void SPDLOG_INLINE
        wincolor_sink<TargetStream, ConsoleMutex>::set_formatter(std::unique_ptr<spdlog::formatter> sink_formatter) {
            std::lock_guard<mutex_t> lock(mutex_);
...
// print a range of formatted message to console
        template<typename TargetStream, typename ConsoleMutex>
        void SPDLOG_INLINE
        wincolor_sink<TargetStream, ConsoleMutex>::print_range_(const fmt::memory_buffer &formatted, size_t start,
                                                                size_t end) {
