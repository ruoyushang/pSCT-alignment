#ifndef ALIGNMENT_GLOBALALIGNMENTOBJECT_HPP
#define ALIGNMENT_GLOBALALIGNMENTOBJECT_HPP
...
class GlobalAlignmentObject : public PasObject {
    UA_DISABLE_COPY(GlobalAlignmentObject);

public:
    GlobalAlignmentObject(
            const UaString &name,
    ...
    UaNodeId typeDefinitionId() const override { return UaNodeId(PAS_GlobalAlignmentType, browseName().namespaceIndex()); }

    const std::map<OpcUa_UInt32, std::tuple<std::string, UaVariant, OpcUa_Boolean, OpcUa_Byte>>
    getVariableDefs() override { return GlobalAlignmentObject::VARIABLES; }

    const std::map<OpcUa_UInt32, std::tuple<std::string, UaVariant, OpcUa_Boolean>>
    getErrorDefs() override { return GlobalAlignmentObject::ERRORS; }

    const std::map<OpcUa_UInt32, std::pair<std::string, std::vector<std::tuple<std::string, UaNodeId, std::string>>>>
    getMethodDefs() override { return GlobalAlignmentObject::METHODS; }
...
//
#endif //ALIGNMENT_GLOBALALIGNMENTOBJECT_HPP
