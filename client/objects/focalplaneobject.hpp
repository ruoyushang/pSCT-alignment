//
// Created by Deivid Ribeiro on 3/10/20.
//

#ifndef ALIGNMENT_FOCALPLANEOBJECT_HPP
#define ALIGNMENT_FOCALPLANEOBJECT_HPP
...
class FocalPlaneObject : public PasObject
{
    UA_DISABLE_COPY(FocalPlaneObject);
public:
    FocalPlaneObject(
            const UaString& name,
    ...
    UaNodeId typeDefinitionId() const override { return UaNodeId(PAS_FocalPlaneType, browseName().namespaceIndex()); }

    const std::map<OpcUa_UInt32, std::tuple<std::string, UaVariant, OpcUa_Boolean, OpcUa_Byte>>
    getVariableDefs() override { return FocalPlaneObject::VARIABLES; }

    const std::map<OpcUa_UInt32, std::tuple<std::string, UaVariant, OpcUa_Boolean>>
    getErrorDefs() override { return FocalPlaneObject::ERRORS; }

    const std::map<OpcUa_UInt32, std::pair<std::string, std::vector<std::tuple<std::string, UaNodeId, std::string>>>>
    getMethodDefs() override { return FocalPlaneObject::METHODS; }
...
#endif //ALIGNMENT_FOCALPLANEOBJECT_HPP
