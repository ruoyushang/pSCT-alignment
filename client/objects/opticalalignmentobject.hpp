...
#ifndef ALIGNMENT_OPTICALALIGNMENTOBJECT_HPP
#define ALIGNMENT_OPTICALALIGNMENTOBJECT_HPP
...
class OpticalAlignmentObject : public PasObject {
    UA_DISABLE_COPY(OpticalAlignmentObject);

public:
    OpticalAlignmentObject(
        const UaString &name,
    ...
    getErrorDefs() override { return OpticalAlignmentObject::ERRORS; }

    const std::map<OpcUa_UInt32, std::pair<std::string, std::vector<std::tuple<std::string, UaNodeId, std::string>>>>
    getMethodDefs() override { return OpticalAlignmentObject::METHODS; }
...
#endif //ALIGNMENT_OPTICALALIGNMENTOBJECT_HPP
