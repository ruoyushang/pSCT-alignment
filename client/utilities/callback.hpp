        ...
        for (i = 0; i < dataNotifications.length(); i++) {
            if (OpcUa_IsGood(dataNotifications[i].Value.StatusCode)) {
                ...
                if (tempValue.type() == OpcUaType_ExtensionObject) {
                    printExtensionObjects(tempValue, UaString("Variable %1").arg(
                        (unsigned int) dataNotifications[i].ClientHandle));
                } else {
...
public:
    ...
    virtual void newEvents(
        ...
        for (i = 0; i < eventFieldList.length(); i++) {
            ...
            if (OpcUa_IsGood(UaVariant(eventFieldList[i].EventFields[1]).toNodeId(newEvent.eventType)) &&
                OpcUa_IsGood(UaVariant(eventFieldList[i].EventFields[14]).toNodeId(newEvent.conditionNodeId))) {
                ...
                if (newEvent.eventType == OpcUaId_RefreshStartEventType) {
                    ...
                    printf(
                        "Event[%d] Skipped - can not get event type of conditionId - SourceName = %s Message = %s \n",
                        i,
                        sourceName.toString().toUtf8(),
                        message.toString().toUtf8());
                }
